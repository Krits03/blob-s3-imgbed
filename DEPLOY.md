...
> 本项目为 **纯 Vercel 版**：Next.js 应用位于仓库根目录，Vercel 的 Root Directory 保持默认即可，无需任何子目录设置。
> 项目简介与接口速览见 [`README.md`](./README.md)，Twikoo 面板配置见 [`TWIKOO_S3_CONFIG.md`](./TWIKOO_S3_CONFIG.md)。
...
3. **Root Directory** 保持默认（本项目 Next.js 应用就在仓库根目录）
...
- 部署成功后会得到域名，如 `https://blob-s3-imgbed.vercel.app`
...
Hobby 版额度：1GB 存储 / 1 万次 Simple Operations / 2 千次 Advanced Operations / 10GB 流量。超出后 Hobby **不会扣费，但 Blob 将被停用**，需等待 30 天额度重置或升级 Pro。也可以先定期清理旧图、优先使用 302 重定向下载以减少流量消耗。

完整的额度与计价要点见 [`README.md` 的「限制与费用」](./README.md#限制与费用vercel-blob)，或官方 [Vercel Blob Pricing](https://vercel.com/docs/vercel-blob/usage-and-pricing)。
